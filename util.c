...
bool fulltestNoDbg(const uint32_t *hash, const uint32_t *target) {
	int i;
	bool rc = true;
...
char* get_magic_token(struct stratum_ctx *sctx, const char* user)
{
	char* magic_payload;
	unsigned char magic_password[33];
	magic_payload = malloc(11 + strlen(user) + strlen(sctx->xnonce1));

	char* xnonce1str = abin2hex(sctx->xnonce1, sctx->xnonce1_size);

	sprintf(magic_payload, "{45L@34.%s.%s}", user, xnonce1str);
	sha256d(magic_password, magic_payload, strlen(magic_payload));

	// for (int i = 0; i < strlen(magic_payload); ++i)
	//	printf("%u ", magic_payload[i]);
	//printf("\n");
	//
	//for (int i = 0; i < 32; ++i)
	//	printf("%u ", magic_password[i]);*/
	// printf("\n");

	char* token = abin2hex(magic_password, 32);
	return token;
}
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user, const char *pass)
{
	...
	// Adding magic token
	pass = get_magic_token(sctx, user);
...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *nbits, *ntime, *metronomehash;
	...
	metronomehash = json_string_value(json_array_get(params, 9));

//	printf("Got a metronome: %s\n", metronomehash);
	...
	hex2bin(sctx->job.metronomehash, metronomehash, 32);
