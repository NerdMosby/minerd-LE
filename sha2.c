	...
	const uint32_t *ptarget, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	sha256d_prehash(prehash, pdata + 24);
	for (i = 7; i >= 0; i--) {
		for (j = 0; j < 4; j++) {
...
/*#ifdef HAVE_SHA256_8WAY
	...
	do {
		pdata[27] = n;
		sha256d(hash, pdata, 28 * sizeof(uint32_t));

		if (fulltestNoDbg(hash, ptarget, false)) {
			*hashes_done = n - first_nonce + 1;
			// printf("Yessss! Nonce: %u\n", n);
			return 1;
		}

		if (n == 0) {
			/*printf("Input = ");
			for (int kkk = 0; kkk < 28; ++kkk) {
				printf("%08x", pdata[kkk]);
			}
			printf("\n");*/
			/*printf("Hash = ");
			...
			/*memcpy(oldhash, hash, 8 * sizeof(uint32_t));
