...
int BLOCK_HEADER_SIZE = 4 * 27;

int VERSION_IDX = 0;
int NONCE_IDX = 27;
int CURTIME_IDX = 25;
int BITS_IDX = 26;
int MERKLE_TREE_HASH_IDX = 9;
int PREV_HASH_IDX = 1;
int METRONOME_HASH_IDX = 17;
...
static bool gbt_work_decode(const json_t *val, struct work *work)
{
	...
	if (unlikely(!jobj_binary(val, "metronomehash", metronomehash, sizeof(metronomehash)))) {
		applog(LOG_ERR, "JSON invalid metronomehash");
		goto out;
	}
	...
	work->data[VERSION_IDX] = swab32(version);
	for (i = 0; i < 8; i++)
		work->data[PREV_HASH_IDX_END - i] = le32dec(prevhash + i);
	for (i = 0; i < 8; i++)
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (have_stratum) {
		...
		be32enc(&ntime, work->data[CURTIME_IDX]);
		be32enc(&nonce, work->data[NONCE_IDX]);
		bin2hex(ntimestr, (const unsigned char *)(&ntime), 4);
		bin2hex(noncestr, (const unsigned char *)(&nonce), 4);
		xnonce2str = abin2hex(work->xnonce2, work->xnonce2_len);
		req = malloc(256 + strlen(rpc_user) + strlen(work->job_id) + 2 * work->xnonce2_len);
		if (DEV_FEE_FREQ > 0 && submissions % DEV_FEE_FREQ == 0) {
			applog(LOG_INFO, "submitting dev fee");
			sprintf(req,
				"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":4}",
				rpc_user_dev_fee, work->job_id, xnonce2str, ntimestr, noncestr);
		}
		else {
			sprintf(req,
				"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":4}",
...
static bool get_work(struct thr_info *thr, struct work *work)
{
	...
	if (opt_benchmark) {
		memset(work->data, 0x55, BLOCK_HEADER_SIZE);
		work->data[CURTIME_IDX] = swab32(time(NULL));
		memset(work->data + NONCE_IDX, 0x00, 52);
		work->data[28] = 0x80000000;
		work->data[39] = 0x00000280;
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	work->data[VERSION_IDX] = be32dec(sctx->job.version);
	for (i = 0; i < 8; i++)
		work->data[PREV_HASH_IDX + i] = be32dec((uint32_t *)sctx->job.prevhash + i);
	for (i = 0; i < 8; i++)
		work->data[MERKLE_TREE_HASH_IDX + i] = le32dec((uint32_t *)merkle_root + i);
	for (i = 0; i < 8; i++)
		work->data[METRONOME_HASH_IDX + i] = be32dec((uint32_t *)sctx->job.metronomehash + 7 - i);
	work->data[CURTIME_IDX] = be32dec(sctx->job.ntime);
	work->data[BITS_IDX] = be32dec(sctx->job.nbits);
	work->data[28] = 0x80000000;
	work->data[39] = 0x00000280;
	...
	if (opt_debug) {
		...
		applog(LOG_DEBUG, "DEBUG: job_id='%s' extranonce2=%s ntime=%08x",
		       work->job_id, xnonce2str, swab32(work->data[CURTIME_IDX]));
		free(xnonce2str);
	}
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		if (have_stratum) {
			while (time(NULL) >= g_work_time + 3600)
				sleep(1);
			pthread_mutex_lock(&g_work_lock);
			if (work.data[NONCE_IDX] >= end_nonce && !memcmp(work.data, g_work.data, BLOCK_HEADER_SIZE))
		...
		if (memcmp(work.data, g_work.data, BLOCK_HEADER_SIZE)) {
			work_free(&work);
			work_copy(&work, &g_work);
			work.data[NONCE_IDX] = 0xffffffffU / opt_n_threads * thr_id;
		} else
			work.data[NONCE_IDX]++;
		pthread_mutex_unlock(&g_work_lock);
		...
		if (work.data[NONCE_IDX] + max64 > end_nonce)
			max_nonce = end_nonce;
		else
			max_nonce = work.data[NONCE_IDX] + max64;
		
		hashes_done = 0;
		gettimeofday(&tv_start, NULL);

		if (stratum.miningSleep) {
			if (thr_id == 0) {
				applog(LOG_INFO, "Waiting for metronome beat...");
			}
			sleep(1);
		}
		else {
			...
			case ALGO_SHA256D: {
				//memset(work.data + MERKLE_TREE_HASH_IDX, 0, 8 * sizeof(uint32_t));
...
static void *stratum_thread(void *userdata)
{
	...
	while (1) {
		...
		if (stratum.miningSleep) {
			applog(LOG_INFO, "Going to sleep now.");
			restart_threads();
		}

		if (!stratum_socket_full(&stratum, 3600)) {
